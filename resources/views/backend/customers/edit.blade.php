    ...
    <div class="row">
        ...
        <div class="col-sm">
            <div class="card card-border-color card-border-color-primary">
                <div class="card-header card-header-divider">Edit customer
                    @if(Session::has('success'))
                    <div class="alert alert-success p-3 mt-1">{{ session('success') }}</div>
                    @endif
                </div>
                <div class="card-body">
                    <form id="form1" action="{{ route('customers.update', ['id' => $customer->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        
                        <div class="form-group pt-2">
                            <label for="name">Name</label>
                            <input class="form-control @error('name') is-invalid @enderror" id="name" type="text" placeholder="Enter the name" name="name" value="{{ $customer->name }}">
                            @error('name')
                        ...
                        <div class="form-group pt-2">
                            ...
                            <input class="form-control @error('job') is-invalid @enderror" id="job" type="text" placeholder="Enter the job" name="job" value="{{ $customer->job }}">
                            @error('job')
                            <span class="text-danger"></span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group pt-2">
                            ...
                            <textarea rows="4" name="comment" id="comment" class="form-control @error('comment') is-invalid @enderror">{!! $customer->comment !!}</textarea>
                            @error('comment')
